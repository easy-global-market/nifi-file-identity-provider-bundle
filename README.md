...
1. Build the provider NAR file or use a [released version](https://github.com/easy-global-market/nifi-file-identity-provider-bundle/releases)
2. Deploy the provider NAR file to your NiFi installation
3. Configure NiFi for HTTPS
4. Configure the File Authorization Provider (*conf/login-identity-providers.xml*)
5. Configure the identity of the File Authorization Provider in *nifi.properties*
6. Add users and their Bcrypt-hashed passwords in *conf/login-credentials.xml*
...
NiFi must be configured for HTTPS, including at least the following settings in *conf/nifi.properties*:
* nifi.web.https.port
...
Add the following Login Identity Provider in *conf/login-identity-providers.xml*:
...
After the provider itself is configured, reference it in *conf/nifi.properties*.
...
Add an entry in the previously created *conf/login-credentials.xml* for the first user, for instance:
...
Then configure it as the initial admin of the NiFi instance in *conf/authorizers.xml*. This has to be done in:
